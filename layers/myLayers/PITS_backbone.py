...
from layers.myLayers.PITS_layers import *
...
class PITS_backbone(nn.Module):
    ...
    def forward(self, z): #(266, 7, 432)
        ...
        if self.padding_patch == 'end':
            z = self.padding_patch_layer(z) #(256, 7, 444)
        z = z.unfold(dimension=-1, size=self.patch_len, step=self.stride)  #(256, 7, 36, 24) (B, C, patch_sum, patch_len)
        z = z.permute(0,1,3,2)  #(256, 7, 24, 36) (B, C, patch_len, num_patch)
...
class Flatten_Head(nn.Module):
    ...
    def forward(self, x): #(256, 7, 128, 36)                                # x: [bs x nvars x d_model x patch_num]
        if self.individual:
            x_out = []
            for i in range(self.n_vars):
                z = self.flattens[i](x[:,i,:,:]) #(256, 4608) # z: [bs x d_model * patch_num]
                z = self.linears[i](z)   #(256, 336)                # z: [bs x target_window]
                z = self.dropouts[i](z)
                x_out.append(z)
            x = torch.stack(x_out, dim=1)#(256, 7, 336)            # x: [bs x nvars x target_window]
        else:
            x = self.flatten(x)
            x = self.linear(x)
            x = self.dropout(x)
...
class FC2Encoder(nn.Module):
    ...
    def forward(self, x) -> Tensor: #(256, 7, 24, 36)
        """
        x: tensor [bs x num_patch x nvars x patch_len]
        # [128, 7, 12, 56]
        """
        x = x.permute(0,3,1,2)  #(256, 36, 7, 24) (bs, num_patch, n_vars, patch_len)
        bs, num_patch, n_vars, patch_len = x.shape
        # Input encoding
        if not self.shared_embedding:
            x_out = []
            for i in range(n_vars):
                z = self.W_P1[i](x[:,:,i,:]) #(256, 36, 128)
                x_out.append(z)
                z = self.act(z)
                z = self.W_P2[i](z) # ??
            x = torch.stack(x_out, dim=2) #(256, 36, 7, 128)
        else:
            x = self.W_P1(x)                                                     
            x = self.act(x)
            x = self.W_P2(x)                                                     
        x = x.transpose(1,2) #(256, 7, 36, 128)
        x = x.permute(0,1,3,2) #(256, 7, 128, 36)
        return x