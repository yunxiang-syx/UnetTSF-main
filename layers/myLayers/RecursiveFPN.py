...
class RecursiveFPN(nn.Module):
    ...
    def forward(self, x):  # x:(256,432,7)
        ...
        for down_block in self.down_blocks:
            ...
            i = i + 1
        # e_out{(256,7,336), (256,7,167), (256,7,83)}
        e_last = e_out[self.stage_num - 1]  # (256,7,83)
        e_middle = []
        e_middle.insert(0, e_out[self.stage_num - 1])
        for i in range(self.stage_num - 1):
            e_last = torch.cat((e_out[self.stage_num - i - 2], e_last), dim=2)  # (256,7,250)
            e_last = self.up_blocks[i](e_last)  # (256,7,167)
            e_middle.insert(0, e_last)
        #e_middle{(256,7,336), (256,7,167), (256,7,83)}
        #开始第二轮
        e_out = []
        i = 0
        for down_block in self.down_blocks:
            e_out.append(down_block(x2))
            x2 = self.Maxpools[i](x2)
            i = i + 1
        # e_out{(256,7,336), (256,7,167), (256,7,83)}
        e_right = []
        #第一层和第二层直接相加
        for i in range(len(e_out)):
            e_right.append(e_out[i] + e_middle[i])
        # e_right{(256,7,336), (256,7,167), (256,7,83)}
        e_last = e_right[self.stage_num - 1]
        for i in range(self.stage_num - 1):
            e_last = torch.cat((e_right[self.stage_num - i - 2], e_last), dim=2)  # (256,7,250)
            e_last = self.up_blocks[i](e_last)  # (256,7,167)
            e_middle.insert(0, e_last)
